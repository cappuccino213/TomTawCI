...
@router.post("/build-evaluation", name="评估提测质量", response_model=bug_related_schemas.BuildQualityEvaluation)
async def get_build_quality(bug_severity_num: bug_related_schemas.BugSeverityNum):
	...
	bqe = bug_related_schemas.BuildQualityEvaluation(IN_value=in_value, evaluate_grade=grade, release_evaluation=release)
	if bqe:
