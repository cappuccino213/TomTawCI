...
def email_notice_rule(notice_type: str, business_id: int, server_account: str, server_password: str,
                      email_accounts: list[str], if_review):
    """
    ...
    # 发布单邮件
        ...
        if if_review:
            ...
            email_cc.extend(user_model.get_user_email(dict(dept=8)))  # 从部门为【发布评审】里面的获取牟的邮箱
            # email_cc = [('九层风', '1483029082@qq.com')]  # 测试数据

            # 增加审批单作为附件
            product_code = db_release.name.split(' ')[0]  # name的返回值必须为eWordRIS V1.2.4.5096.RTX.b20230919带空格
            # 获取发布年份
            release_year = db_release.date.strftime("%Y")
            attachment_path = os.path.join(AUTO_DISTRIBUTE['COMPRESS_PATH'], product_code, release_year, 'ApprovalForm',
                                           f"{db_release.name}发布审批单.pdf")
            email_struct['attachments'] = attachment_path
