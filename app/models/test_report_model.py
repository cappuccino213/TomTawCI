...
class TestReportModel(Base):
    ...
    # 多条件查询测试报告信息
    @classmethod
    def query_report_info(cls, condition: dict):
        try:
            ...
            if condition.get('project'):
                result = result.filter(cls.product == condition.get('project'))
            if condition.get('tasks'):
                result = result.filter(cls.tasks == condition.get('tasks'))
            return result.first()
